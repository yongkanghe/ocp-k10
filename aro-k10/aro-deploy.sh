...
az group create \
  --name $MY_PREFIX-$ARO_MY_GROUP \
  --location $ARO_MY_LOCATION

az network vnet create \
  --resource-group $MY_PREFIX-$ARO_MY_GROUP \
  --name aro-vnet4yong1 \
...
PASSWORD=$(az aro list-credentials --name $ARO_MY_CLUSTER --resource-group $MY_PREFIX-$ARO_MY_GROUP -o tsv --query kubeadminPassword)

apiServer=$(az aro show -g $MY_PREFIX-$ARO_MY_GROUP -n $ARO_MY_CLUSTER --query apiserverProfile.url -o tsv)
